...
class BatchQuery:
    ...
    def b_query(self):
        ...
        for rownum in range(1,rowmax+1,1):
            ...
            if rowmax==1 and chem_i is None:
                st.write('您上传的文件是空白的，请检查！')
                return None
...
st.set_page_config(page_title="A bridge to Chemical Compliance",layout="wide")
...
st.text('or')
uploaded_xlsx_file=st.file_uploader('批量查询 Bulk Query（上传文件格式为xlsx且第一列为CAS no）',type=['xlsx'])
...
if whichcasno!='':
    if findillegalchar(whichcasno):
        ...
        if df_result_0 is not None:
            ...
            st.write('oops!未检索到关联法规数据 no any match!')   
